...
function surface(overrides: Partial<ReadingSurface> = {}): ReadingSurface {
  return {
    ...
    mainText: [
      ...
      "Runtime fixture reports one synthetic claim.",
...
describe("sidepanel page reading runtime", () => {
  ...
  it("auto-generates a session-only General Page brief when Tier B is available", async () => {
    ...
    const backgroundSection = pagePaneEl.querySelector(".page-reader-analysis-summary + .page-reader-analysis-section");
    expect(backgroundSection?.classList.contains("is-single")).toBe(false);
    expect(backgroundSection?.querySelector("p")).toBeNull();
    expect(backgroundSection?.querySelectorAll("ul > li")).toHaveLength(1);
  ...
  it("shows automatic investigation preparation and keeps URL as AI Mode metadata", async () => {
    ...
    expect(aiModeQuery).toContain("原文陳述：Runtime fixture reports one synthetic claim.");
    const needToggle = pagePaneEl.querySelector<HTMLButtonElement>(".page-claim-evidence-toggle");
    needToggle?.click();
    expect(needToggle?.getAttribute("aria-expanded")).toBe("true");
    expect(readyCard?.classList.contains("is-evidence-open")).toBe(true);
  ...
  it("renders Gemini, copy, and evidence disclosure only for Adapter-approved outcomes", async () => {
        ...
        return {
          ...
          brief: {
            ...
            claims: [0, 1, 2].map((index) => ({
              c: `Synthetic claim ${index + 1}.`,
              why: `第 ${index + 1} 項影響公共判斷。`,
              need: `第 ${index + 1} 項權威資料。`,
              q: `第 ${index + 1} 項主張是否有外部證據支持？`,
            })),
            model: "brief-model",
          },
        } satisfies TrulyMessage;
    ...
    runtime.handleGeneralPageInvestigationResult({
      ...
      preparedClaim: {
        ...
        need: "An authoritative record.",
        q: "Does Runtime fixture report one synthetic claim?",
        displayQ: "Does Runtime fixture report one synthetic claim?",
        atom: { s: "Runtime fixture", p: "reports", o: "one synthetic claim" },
        policy: { claimKind: "fact", consequence: "public_interest" },
      },
    });
    ...
    for (const claimIndex of [1, 2]) {
      runtime.handleGeneralPageInvestigationResult({
        ...
        scope: "page",
        claimIndex,
        status: claimIndex === 1 ? "ineligible" : "unavailable",
      });
    }
    ...
    expect(rows.every((row) => row.querySelector(".page-claim-investigation-actions"))).toBe(true);
    for (const row of rows) {
      const investigation = row.querySelector(".page-claim-investigation");
      expect(investigation?.children[0]?.classList.contains("page-claim-investigation-main")).toBe(true);
  ...
  it("fails a stalled investigation batch closed after the bounded UI deadline", async () => {
    vi.useFakeTimers();
    try {
      const pagePaneEl = setupDom();
      let analysisKey = "";
          ...
          return {
            ...
            brief: {
              ...
              claims: [0, 1].map((index) => ({
                c: `Runtime fixture reports synthetic claim ${index + 1}.`,
                why: "It matters.",
                need: "An authoritative record.",
                q: `Does Runtime fixture report synthetic claim ${index + 1}?`,
              })),
              model: "brief-model",
            },
          } satisfies TrulyMessage;
      ...
      runtime.handleGeneralPageInvestigationResult({
        ...
        status: "prepared",
        preparedClaim: {
          c: "Runtime fixture reports synthetic claim 1.",
          why: "It matters.",
          need: "An authoritative record.",
          q: "Does Runtime fixture report synthetic claim 1?",
          displayQ: "Runtime fixture 是否報導第一項合成主張？",
          atom: { s: "Runtime fixture", p: "reports", o: "synthetic claim 1" },
          policy: { claimKind: "fact", consequence: "public_interest" },
        },
      });
      expect(pagePaneEl.querySelectorAll(".page-claim-row")).toHaveLength(0);
      ...
      runtime.handleGeneralPageInvestigationResult({
        ...
        status: "prepared",
        preparedClaim: {
          c: "Runtime fixture reports synthetic claim 2.",
          why: "It matters.",
          need: "An authoritative record.",
          q: "Does Runtime fixture report synthetic claim 2?",
          displayQ: "Runtime fixture 是否報導第二項合成主張？",
          atom: { s: "Runtime fixture", p: "reports", o: "synthetic claim 2" },
          policy: { claimKind: "fact", consequence: "public_interest" },
        },
      });
      expect(pagePaneEl.querySelector(".page-claim-section")).toBeNull();
