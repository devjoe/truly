...
describe("General Page analysis contract", () => {
  ...
  it("attaches a confirmed screenshot as an image part without replacing the text prompt", () => {
    ...
    expect(withoutShot.max_tokens).toBe(700);
    ...
    expect(englishPrompt).toContain("claims must be an empty array");
    expect(englishPrompt).toContain("The independent Investigation Selector owns verification actions");
    expect(englishPrompt).not.toContain("exactly one atomic assertion");
    expect(englishPrompt).not.toContain("atom.s, atom.p, and atom.o");
    ...
    expect(englishPrompt).toContain("claims exactly 0 items");
    ...
    expect(zhPrompt).toContain("claims 必須是空陣列");
    expect(zhPrompt).toContain("獨立的 Investigation Selector 負責查核 action");
    expect(zhPrompt).not.toContain("每個 claim 只能有一個原子主張");
    expect(zhPrompt).not.toContain("atom.s、atom.p、atom.o");
    ...
    expect(zhPrompt).toContain("claims 必須剛好 0 項");
    ...
    expect(withShot.max_tokens).toBe(700);
