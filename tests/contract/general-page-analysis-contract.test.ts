...
describe("General Page analysis contract", () => {
  ...
  it("attaches a confirmed screenshot as an image part without replacing the text prompt", () => {
    ...
    expect(englishPrompt).toContain("underlying real-world fact");
    expect(englishPrompt).toContain("Apply a source-sufficiency test");
    expect(englishPrompt).toContain("Do not merely ask whether the page or speaker stated the claim");
    expect(englishPrompt).toContain("both a concrete primary evidence class and its relevant holder or subject");
    expect(englishPrompt).toContain("at most 14 English words");
    expect(englishPrompt).toContain("Page observations are not claims");
    expect(englishPrompt).toContain("do not add these page-quality observations as extra claims");
    expect(englishPrompt).toContain("select the external fact rather than the reporting act");
    expect(englishPrompt).toContain("Never end with a dangling verb");
    expect(englishPrompt).toContain("what official data exists");
    expect(englishPrompt).toContain("ask what evidence supports a claim");
    expect(englishPrompt).toContain("current official status");
    ...
    expect(zhPrompt).toContain("底層現實事實");
    expect(zhPrompt).toContain("執行來源充分性測試");
    expect(zhPrompt).toContain("不得只問本頁或某人是否說過該主張");
    expect(zhPrompt).toContain("具體的第一手證據類型及其相關持有者或對象");
    expect(zhPrompt).toContain("目標 36 字內");
    expect(zhPrompt).toContain("頁面觀察不是 claim");
    expect(zhPrompt).toContain("不得再把這些頁面品質觀察當成額外 claims");
    expect(zhPrompt).toContain("應選底層事實而非轉述行為");
    expect(zhPrompt).toContain("不得以『用以確認』等懸空動詞結尾");
    expect(zhPrompt).toContain("有哪些官方資料");
    expect(zhPrompt).toContain("不得詢問支持主張的證據");
    expect(zhPrompt).toContain("目前官方狀態");
    expect(zhPrompt).toContain("這些都是查核問題");
