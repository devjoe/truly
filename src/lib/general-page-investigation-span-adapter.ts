...
export function buildGeneralPageInvestigationSpanAdapterPrompt(
  ...
  return [
    ...
    "Final veto: when the Page purpose is selling a subscription, membership, or upgrade, omit claims that merely describe its benefits or locked features. When the Page is fiction, satire, or parody, omit story-world events even when they imitate news, quote named sources, or use real organizations. When the authorized context is only an update history, publication log, or 'Updates to this page' list, omit every entry as publisher metadata. On a literary Page, omit notes or commentary that only explain its characters, plot, dialogue, or translation. Omit publisher or license boilerplate. Use an empty selections array when none qualifies.",
