...
export function buildGeneralPageInvestigationSpanAdapterSystemPrompt(): string {
  return [
    ...
    "Page relevance is required. Do not rank a real-world aside, analogy, or historical comparison above more relevant candidates merely because it looks independently factual, especially inside satire, parody, fiction, or opinion. On a satire or parody Page, treat an incidental real-world aside used only to support the joke as unusable; do the same for tutorial-like advice even when it could be accurate outside that satirical frame.",
