...
export function buildGeneralPageInvestigationSpanAdapterSystemPrompt(): string {
  return [
    ...
    "An article, newsletter, or editor meta-description that merely says an author writes about a topic is not the underlying real-world claim. Prefer a specific body proposition about the named event, count, decision, behavior, or record.",
...
export function buildGeneralPageInvestigationSpanAdapterPrompt(
  ...
  return [
    ...
    "Before comparing utility, mark the whole exact span structurally unusable when it resembles any of these patterns: duplicated title or type text ('NAME NAME states...'); stacked documentation labels ('Usage Enabling NAME...' or 'Option Usage NAME...'); stacked Page sections ('Latest from NAME What we do...'); release header and date before body ('For Immediate Release NUMBER PLACE, DATE — ...'); or an article-about-topic lead ('Today’s article is about...'). Also discard a forward pointer or backward reference ('In the following example...', a sentence starting with 'it' or 'this' without a named subject, or 'as described above'). Reject the whole exact span even when its remaining clause is central; a clean later candidate always outranks it.",
