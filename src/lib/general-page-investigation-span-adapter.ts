...
export function buildGeneralPageInvestigationSpanAdapterSystemPrompt(): string {
  return [
    ...
    "Tiering is absolute, not relative: being the only or strongest usable candidate does not make it primary. A stable catalog or reference fact remains exploratory even when it is specific, named, or dated.",
...
export function buildGeneralPageInvestigationSpanAdapterPrompt(
  ...
  return [
    ...
    "Apply the primary bar independently. Do not label the best available candidate primary merely because no stronger candidate exists.",
