...
export function buildGeneralPageInvestigationSpanAdapterSystemPrompt(): string {
  return [
    ...
    "A dateline/timezone fragment at the start, a chain of colon-delimited navigation labels, publication metadata such as 'From: Published: ... Last updated:', a duplicated heading before its subject, or a page-relative phrase such as 'as described above' or 'set out above' makes the whole exact span unusable.",
...
export function buildGeneralPageInvestigationSpanAdapterPrompt(
  ...
  return [
    ...
    "Before comparing utility, mark the whole exact span structurally unusable when it resembles any of these patterns: duplicated title or type text ('NAME NAME states...'); stacked documentation labels ('Usage Enabling NAME...' or 'Option Usage NAME...'); stacked Page sections ('Latest from NAME What we do...'); release header and date before body ('For Immediate Release NUMBER PLACE, DATE — ...'); publication metadata ('From: Published: ... Last updated:'); a standalone timezone before prose ('EDT The ...'); or an article-about-topic lead ('Today’s article is about...', 'Today, NAME writes about...', or 'We came here to see...'). Also discard a forward pointer or backward reference ('In the following example...', a sentence starting with 'it' or 'this' without a named subject, 'these documents', 'as described above', or 'set out above'). Reject the whole exact span even when its remaining clause is central; a clean later candidate always outranks it.",
