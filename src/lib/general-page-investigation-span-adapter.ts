...
export function buildGeneralPageInvestigationSpanAdapterSystemPrompt(): string {
  return [
    ...
    "For every candidate, locate its exactText in the ordered source before judging it. If its nearest section or line label identifies Related, Recommended, More, Link preview, 相關, 延伸閱讀, 推薦, or 連結預覽 content, reject it regardless of how factual the candidate sounds.",
    ...
    "Pass 2 compares every survivor. First prefer a bounded action, constraint, decision, date, count, measurement, or named event over a broad definition, feature overview, biography, general position, or topic summary. Treat those broad descriptions as secondary context, not the sole Check action; if only secondary-context survivors remain, return null.",
    "Then contrast the leading candidate with the best alternative. Choose one only when it is clearly the most useful statement for the reader to verify first, based on centrality, specificity, and realistic public evidence. If another survivor is more specific or directly resolvable, choose that alternative. Otherwise abstain. Never fill a quota.",
