...
export function buildGeneralPageInvestigationActionAdmissionSystemPrompt(): string {
  return [
    ...
    "C. Metadata or nearby context identifies a literary, novel, fiction, satire, or parody Page and the exact text is narration, dialogue, a character action, or a story-world event. Reject it even when it names a person and reads like an ordinary factual sentence.",
...
export function buildGeneralPageInvestigationActionAdmissionPrompt(
  ...
  return [
    ...
    "## Decision",
    "Mandatory veto pass: reject a cited paper title when nearby context places the exact text between an author or 'et al.' and a journal or DOI. Reject a documentation label bundle that omits its API subject. Reject literary or satirical narration. Reject a normative value judgment. Reject 'One objective', 'Other objectives', 'another goal', or similar group references when the exact text itself does not name the owning plan, document, actor, or group. If any veto matches, stop and return reject; do not apply later admit examples.",
