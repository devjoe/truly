...
export function buildGeneralPageInvestigationActionTierSystemPrompt(): string {
  return [
    ...
    "Mandatory precedence: when the exact proposition only says that a scheduled natural phenomenon or voluntary public gathering will occur, or only supplies its time, place, visibility, audience, viewing, registration, or attendance details, output exploratory. A large audience or spectacular description does not turn the scheduled occurrence itself into a changed status.",
    "Do not apply that event-information rule to an infrastructure closure, service disruption, official detour, newly added product, service, menu item, release, or availability change; those remain primary when central and specific.",
    ...
    "On a current official press release, a central exact count, shortlisting or public survey, newly issued list, enforcement action, rate decision, or named policy change is primary. Do not downgrade that current official action merely because the surrounding Page also explains a stable process.",
...
export function buildGeneralPageInvestigationActionTierPrompt(
  ...
  return [
    ...
    "## Classification",
    "Classify only the investigation utility of this admitted proposition. Apply the informational-event precedence before considering whether the proposition is current or central.",
  ].join("\n");
}
