...
export function buildGeneralPageInvestigationActionTierSystemPrompt(): string {
  return [
    ...
    "Mandatory precedence: when the exact proposition only says that a scheduled natural phenomenon or public gathering will occur, or only supplies its time, place, visibility, audience, viewing, registration, or attendance details, output exploratory. A large audience or spectacular description does not turn the scheduled occurrence itself into a changed status.",
