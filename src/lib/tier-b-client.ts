...
export function generalPageBriefSystemPrompt(
  ...
  if (lang === "en") {
    return [
      ...
      "An ordinary purchase decision is not by itself a consequential financial assertion. Advertised discounts, course length or chapter counts, comparison-table availability, and ordinary product/tool features stay out of claims. A health or safety suitability statement for children, pregnancy, patients, or another vulnerable group remains consequential even inside an advertisement.",
      "A person's single success or failure with a tool is personal experience. Do not generalize it into a product capability claim. If the supplied text is an index, feed, or mixed list of unrelated headlines rather than one coherent target, return claims as an empty array or omit it.",
      "Claims MUST be empty for ordinary engagement or publication metadata such as views, likes, follower counts, or the mere date of a post; for a course's advertised duration, chapters, discount, coupon, or bonus; for a launch post saying a tool solves a workflow problem; and for where an ordinary software preference appears in an interface. Put a useful product/documentation question in qs instead when appropriate.",
  ...
  return [
    ...
    "一般購買決策本身不等於具後果的財務主張。廣告折扣、課程時數或章節數、比較表是否存在、一般產品或工具功能，都不要放入 claims。即使出現在廣告中，若宣稱孩童、孕婦、病患或其他脆弱族群適用，仍屬具後果的健康或安全主張。",
    "單一使用者操作工具成功或失敗，只是個人經驗，不得推廣成產品能力主張。若提供的文字是索引、feed 或互不相關的多則標題清單，而不是單一完整目標，claims 回空陣列或省略。",
    "遇到一般互動或發布資訊，例如觀看數、按讚數、追蹤數或單純的貼文日期；課程宣傳的時數、章節、折扣、折扣碼或贈品；新工具宣稱可解決某個工作流程問題；一般軟體偏好設定出現在介面何處，claims 必須回空陣列。適合時可把產品或文件理解問題放入 qs。",
