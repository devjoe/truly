...
export function generalPageBriefSystemPrompt(
  ...
  if (lang === "en") {
    return [
      ...
      "Every claim must express exactly one atomic assertion and MUST include atom.s, atom.p, and atom.o. Copy each atom value verbatim from claim.c: one subject, one factual relation, one object/outcome. claim.c must contain no second proposition.",
      "claim.q must be one natural question about the same atom, explicitly naming atom.s and atom.o. It must not use vague references, URLs, domains, Markdown, search-engine names, commands, keyword lists, or facts absent from the page. Omit the claim if q is unreliable.",
  ...
  return [
    ...
    "每個 claim 只能有一個原子主張，並必須包含 atom.s、atom.p、atom.o。三者都從 claims.c 原樣複製，分別表示一個主體、一個事實關係、一個受詞或結果；claims.c 不得再包含第二個命題。",
    "claims.q 必須是查核同一 atom 的一個自然問句，明確寫出 atom.s 與 atom.o；不得使用代稱、網址、網域、Markdown、搜尋引擎名稱、操作指令、關鍵字清單或頁面未出現的事實。無法可靠產生 q 就省略 claim。",
