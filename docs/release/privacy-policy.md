...
Truly is a local-first reading assistant for Facebook. Reading analysis runs in
the model environment you choose. Content is sent to external tools only when
you explicitly use those tools.
...
When you use Truly on supported Facebook pages, the extension may process:

- visible post text, shared-post text, link previews, and image/video context;
- Facebook-hosted media URLs or image alt text when needed for reading
  assistance;
